  ...
  return (
    <div>
      ...
      <p style={ps}>
        ...
        年，仅用于区分设备、记录购买记录和使用量配额，不含任何可识别个人身份的信息。
      </p>
      <h3 style={h3s}>简历内容</h3>
      <p style={ps}>
        默认仅存储在您的本地浏览器，不会主动上传。以下功能使用时会临时发送至服务器处理，处理完成后立即释放，不留存：
      </p>
      <ul style={uls}>
        <li style={lis}>
          AI 简历分析、英文简历生成（发送至阿里云通义千问 API）
        </li>
      ...
      <p style={ps}>
        仅记录订单 ID、设备
        ID、套餐类型、购买时间等，不收集银行卡号等金融敏感信息。
      </p>
      <h2 style={h2s}>2. Cookie 与本地存储</h2>
      <p style={ps}>
        我们使用 Cookie 和 localStorage 存储设备标识及购买记录。不使用第三方追踪
        Cookie，不集成任何广告 SDK。
      </p>
      <h2 style={h2s}>3. 第三方服务</h2>
      <ul style={uls}>
        <li style={lis}>
          <strong>阿里云通义千问</strong>：AI 功能底层模型，受阿里云隐私政策约束
        </li>
        <li style={lis}>
...
function TermsContent() {
  return (
    <div>
      ...
      <p style={ps}>
        本产品不要求注册账号。购买记录绑定于浏览器设备标识，更换设备或清除浏览器数据可能导致记录丢失，请妥善保管订单号。
      </p>
...
const FAQ_SECTIONS = [
  {
    ...
    items: [
      ...
      {
        q: "我需要注册账号吗？",
        a: "不需要。简力全开采用设备识别技术，打开网页即可直接使用，所有数据存储在您的本地浏览器中，购买记录也绑定在您的设备上。",
      },
      {
        q: "我的简历数据会被上传到服务器吗？",
        a: "简历内容默认存储在您的本地浏览器（localStorage）中，不会主动上传。仅在使用 AI 分析、PDF 下载等功能时，相关内容会被临时发送至服务器处理，处理完成后立即释放。",
      },
      {
        q: "如何保存简历？",
  ...
  },
  {
    ...
    items: [
      {
        q: "AI 简历分析有哪些功能？",
        a: "AI 分析包含两个模式：1.简历润色 — 不填岗位信息时，AI 逐条审视描述，找出表达薄弱的地方给出加强建议；2.岗位定向优化 — 填入目标职位信息后，AI 分析匹配度（Offer 率评分）、给出改写建议并生成 10 道面试题。所有改动以高亮差异展示，可逐条选择应用。",
      },
      {
        q: "AI 分析在哪里使用？",
      ...
      {
        q: "AI 功能每天有次数限制吗？",
        a: "Pro 订阅用户每天可使用 AI 分析 20 次、生成英文简历 5 次；单次购买用户享有 AI 分析 2 次（终身）；免费用户可在首页免费使用 AI 分析 2 次（终身）。",
      },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      {
        q: "购买记录会丢失吗？",
        a: "购买记录存储在浏览器本地。清除浏览器数据、换设备或换浏览器可能导致丢失。建议购买后截图保存订单号，如需找回可联系客服。",
      },
      {
        q: "学生优惠如何申请？",
        a: '点击定价页或编辑器内的"学生认证"按钮，使用校园邮箱（.edu.cn 或 .edu 结尾）完成验证，即可享全场 5 折，有效期 1 年。',
      },
      {
        q: "支持退款吗？",
