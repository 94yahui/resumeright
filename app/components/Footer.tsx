  ...
  return (
    <div>
      ...
      <ul style={uls}>
        ...
        <li style={lis}>AI 简历分析：简历润色 + 岗位匹配度评分 + 面试题预测，覆盖两种使用场景</li>
        <li style={lis}>一键生成英文简历：智能翻译全部简历内容，保留原始格式与数据</li>
...
function PrivacyContent() {
  return (
    <div>
      ...
      <ul style={uls}>
        <li style={lis}>AI 简历分析、英文简历生成（发送至阿里云通义千问 API）</li>
...
function TermsContent() {
  return (
    <div>
      ...
      <p style={ps}>简力全开提供在线简历制作、AI 简历分析、英文简历生成等工具服务，分免费版和付费版两种方案。</p>
      ...
      <p style={ps}>本产品页面（首页、模板预览等）所展示的示例简历中，"陈梦瑶"等人名均为虚构演示人物，不代表任何真实存在的自然人，亦不构成对任何个人姓名权或肖像权的侵犯。示例照片为 AI 生成的虚拟形象，不对应任何真实人物。如有相关方认为存在侵权，请联系我们，我们将在核实后第一时间处理。</p>
      <h2 style={h2s}>7. 免责声明</h2>
      <p style={ps}>AI 分析建议仅供参考，不代表专业 HR 意见。我们不对因使用本产品生成的简历导致的求职结果承担责任。</p>
...
const FAQ_SECTIONS = [
  {
    cat: '基础使用',
    items: [
      { q: '简力全开是免费使用的吗？', a: '是的，基础功能完全免费：5 套基础模板、在线编辑、PDF 下载（含水印）、模块拖拽排序、保存无限份简历，以及 2 次终身免费 AI 简历分析。去除水印、全部模板、AI 优化等高级功能需升级 Pro 或单次购买。' },
      { q: '我需要注册账号吗？', a: '不需要。简力全开采用设备识别技术，打开网页即可直接使用，所有数据存储在您的本地浏览器中，购买记录也绑定在您的设备上。' },
      { q: '我的简历数据会被上传到服务器吗？', a: '简历内容默认存储在您的本地浏览器（localStorage）中，不会主动上传。仅在使用 AI 分析、PDF 下载等功能时，相关内容会被临时发送至服务器处理，处理完成后立即释放。' },
  ...
  },
  {
    cat: 'AI 功能',
    items: [
      { q: 'AI 简历分析有哪些功能？', a: 'AI 分析包含两个模式：1.简历润色 — 不填岗位信息时，AI 逐条审视描述，找出表达薄弱的地方给出加强建议；2.岗位定向优化 — 填入目标职位信息后，AI 分析匹配度（Offer 率评分）、给出改写建议并生成 10 道面试题。所有改动以高亮差异展示，可逐条选择应用。' },
      { q: 'AI 分析在哪里使用？', a: '有两个入口：1.首页「分析我的简历」板块，免费用户终身可用 2 次；2.编辑器顶栏「AI 分析」按钮，分析结果可一键应用到当前简历，为 Pro 专属功能。' },
      { q: 'AI 功能每天有次数限制吗？', a: 'Pro 订阅用户每天可使用 AI 分析 20 次、生成英文简历 5 次；单次购买用户享有 AI 分析 2 次（终身）；免费用户可在首页免费使用 AI 分析 2 次（终身）。' },
    ],
  },
  {
    cat: '下载与导出',
