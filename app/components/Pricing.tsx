...
const plans = [
  ...
  },
  {
    ...
    price: '19',
    period: '/ 月 · 按月订阅',
    features: ['全部 50+ 套模板', '无水印下载', 'AI 内容优化（无限次）', '简历智能解析', '岗位匹配分析', '多份简历保存', 'Word / PDF 格式'],
    cta: '订阅 Pro',
    ctaHref: '#',
    featured: true,
  },
    ...
    <section id="pricing" style={{
      ...
      <div style={{ textAlign: 'center' }} className="fade-in">
        ...
        <h2 style={{ fontFamily: "'Inter', 'Noto Sans SC', sans-serif", fontSize: 'clamp(26px, 4vw, 36px)', letterSpacing: '-1px' }}>
          选择适合你的<em style={{ fontStyle: 'italic', color: 'var(--gold)' }}>方案</em>
        </h2>
        <p style={{ color: 'var(--ink3)', marginTop: '10px', fontWeight: 300 }}>灵活选择，按需付费</p>
      </div>
...
function PricingCard({ plan, delay }: { plan: typeof plans[0], delay: number }) {
  ...
  return (
    <div
      className={`fade-in${plan.featured ? ' pricing-featured' : ''}`}
      ...
      style={{
        ...
        borderRadius: '16px',
        padding: '32px 28px',
        border: `1.5px solid ${plan.featured ? 'var(--ink)' : 'var(--paper3)'}`,
        position: 'relative',
    ...
    >
      ...
      <ul style={{ listStyle: 'none', marginBottom: '28px' }}>
        {plan.features.map(f => (
          <li key={f} style={{
            ...
            <span style={{ color: plan.featured ? 'var(--gold2)' : 'var(--teal)', fontWeight: 700, fontSize: '12px' }}>✓</span>
            {f}
          </li>
        ))}
      </ul>
      ...
      <Link href={plan.ctaHref} style={{
        ...
        border: `1.5px solid ${plan.featured ? 'var(--gold)' : 'var(--paper3)'}`,
      }}
      onMouseEnter={e => { if (plan.featured) e.currentTarget.style.background = 'var(--gold2)' }}
      onMouseLeave={e => { if (plan.featured) e.currentTarget.style.background = 'var(--gold)' }}
      >{plan.cta}</Link>
    </div>
  )
}
